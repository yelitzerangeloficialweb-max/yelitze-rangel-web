...
import { DINERO_QUESTIONS, DineroOption } from '@/lib/dinero-data';

interface StepQuizProps {
    onComplete: (answers: Record<number, DineroOption>) => void;
}

export default function StepQuiz({ onComplete }: StepQuizProps) {
    const [currentIndex, setCurrentIndex] = useState(0);
    const [answers, setAnswers] = useState<Record<number, DineroOption>>({});
    const [direction, setDirection] = useState(1);

    const currentQuestion = DINERO_QUESTIONS[currentIndex];
    const progress = ((currentIndex + 1) / DINERO_QUESTIONS.length) * 100;

    const handleOptionSelect = (option: DineroOption) => {
        const newAnswers = { ...answers, [currentQuestion.id]: option };
        setAnswers(newAnswers);

        setTimeout(() => {
            if (currentIndex < DINERO_QUESTIONS.length - 1) {
                setDirection(1);
        ...
        <div className="w-full max-w-2xl mx-auto px-4 min-h-[500px]">
            ...
            <div className="mb-8">
                <div className="flex justify-between text-xs font-bold uppercase tracking-widest text-gray-400 mb-2">
                    <span>Pregunta {currentIndex + 1} de {DINERO_QUESTIONS.length}</span>
