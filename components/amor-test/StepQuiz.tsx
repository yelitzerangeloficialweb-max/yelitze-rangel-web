...
import { AMOR_QUESTIONS, AmorOption } from '@/lib/amor-data';

interface StepQuizProps {
    onComplete: (answers: Record<number, AmorOption>) => void;
}

export default function StepQuiz({ onComplete }: StepQuizProps) {
    const [currentIndex, setCurrentIndex] = useState(0);
    const [answers, setAnswers] = useState<Record<number, AmorOption>>({});
    const [direction, setDirection] = useState(1);

    const currentQuestion = AMOR_QUESTIONS[currentIndex];
    const progress = ((currentIndex + 1) / AMOR_QUESTIONS.length) * 100;

    const handleOptionSelect = (option: AmorOption) => {
        const newAnswers = { ...answers, [currentQuestion.id]: option };
        setAnswers(newAnswers);

        setTimeout(() => {
            if (currentIndex < AMOR_QUESTIONS.length - 1) {
                setDirection(1);
        ...
        <div className="w-full max-w-2xl mx-auto px-4 min-h-[500px]">
            ...
            <div className="mb-8">
                <div className="flex justify-between text-xs font-bold uppercase tracking-widest text-gray-400 mb-2">
                    <span>Pregunta {currentIndex + 1} de {AMOR_QUESTIONS.length}</span>
