...
export default function EventPage({ params }: { params: Promise<{ slug: string }> }) {
    ...
    return (
        <main className="bg-[#fafcfe] min-h-screen pb-20">
            {/* Split-Panel Header - Perfectly Proportioned */}
            <section className="relative flex flex-col lg:flex-row overflow-hidden bg-[#1a1a1a]">
                    ...
                    <FadeIn>
                        ...
                        <h1 className="text-white text-4xl md:text-5xl lg:text-7xl font-heading mb-10 leading-tight">
                            {event.title.includes('®') ? (
                                <>
                                    {event.title.split('®')[0]}<span className="text-2xl lg:text-4xl align-top opacity-60 ml-1">®</span>
                                </>
                            ) : event.title}
                        </h1>
                        ...
                        <div className="flex flex-wrap gap-6 text-sm text-white/70">
                            <div className="flex items-center gap-2">
                                ...
                                <span className="font-medium tracking-widest uppercase text-[10px]">{event.date}</span>
                            </div>
                            {event.time && (
                                <div className="flex items-center gap-2">
                                    <Clock className="w-5 h-5 text-[var(--color-secondary)]" />
                                    <span className="font-medium tracking-widest uppercase text-[10px]">{event.time}</span>
                                </div>
                            )}
                            <div className="flex items-center gap-2">
                                <MapPin className="w-5 h-5 text-[var(--color-secondary)]" />
                                <span className="font-medium tracking-widest uppercase text-[10px]">{event.location}</span>
                            </div>
                        </div>
                    </FadeIn>
                </div>
