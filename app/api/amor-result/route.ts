...
export async function POST(req: Request) {
    try {
        ...
        // Sanitize and trim inputs against XSS and limit lengths
        const cleanName = name ? name.replace(/<[^>]*>/g, '').trim().substring(0, 100) : 'Anónimo';
        const cleanEmail = body.email ? body.email.replace(/<[^>]*>/g, '').trim().toLowerCase().substring(0, 100) : '';
        const cleanDominant = dominantBelief.replace(/<[^>]*>/g, '').trim().substring(0, 100);
        const cleanSecondary = secondaryBelief ? secondaryBelief.replace(/<[^>]*>/g, '').trim().substring(0, 100) : undefined;

        console.log(`Análisis Amor para ${cleanName} - Detectado: ${cleanDominant}`);
...
Creencia/Arquetipo dominante detectado: ${cleanDominant}
Creencia secundaria (si aplica): ${cleanSecondary || "N/A"}
Nombre del usuario: ${cleanName}
...
`;
        ...
        if (cleanEmail && cleanEmail.includes('@')) {
            try {
                console.log(`[Amor Test Debug] Attempting email to: ${cleanEmail}, name: ${cleanName}`);
                ...
                const pdfBuffer = await generateGenericTestPDF(cleanName, 'Creencias sobre el Amor', pdfContent);
                
                const mailResult = await sendGenericTestEmail({
                    email: cleanEmail,
                    name: cleanName,
                    testTitle: 'Creencias sobre el Amor',
                    analysis: parsedResult.screen_message || 'Ver PDF adjunto para el análisis completo.',
                    pdfBuffer
                });
                
                console.log(`[Amor Test Debug] Mail Result:`, JSON.stringify(mailResult));
            } catch (mailError) {
                console.error('[Amor Test Mail Error]:', mailError);
            }
        } else {
            console.warn(`[Amor Test Debug] Skipping email: invalid or missing email address: "${cleanEmail}"`);
        }
