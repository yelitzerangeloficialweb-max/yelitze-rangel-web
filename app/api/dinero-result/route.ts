...
export async function POST(req: Request) {
    try {
        ...
        const cleanEmail = (email || body.email) ? (email || body.email).replace(/<[^>]*>/g, '').trim().toLowerCase().substring(0, 100) : '';
        ...
        const cleanNervous = nervousSystem ? nervousSystem.replace(/<[^>]*>/g, '').trim().substring(0, 100) : 'Desconocido';

        console.log(`Análisis Dinero para ${cleanName} - Detectado: ${cleanDominant}`);
...
- Nombre: ${cleanName}
- Creencia dominante: ${cleanDominant}
- Creencia secundaria: ${cleanSecondary || "N/A"}
- Sistema nervioso: ${cleanNervous}
...
`;
        ...
        if (cleanEmail && cleanEmail.includes('@')) {
            try {
                console.log(`[Dinero Test Debug] Attempting email to: ${cleanEmail}, name: ${cleanName}`);
                ...
                const pdfBuffer = await generateGenericTestPDF(cleanName, 'Relación con el Dinero', pdfContent);
            ...
            console.warn(`[Dinero Test Debug] Skipping email: invalid or missing email address: "${cleanEmail}"`);
        }
