...
export async function POST(req: Request) {
    try {
        ...
        const cleanWound = dominantWound.replace(/<[^>]*>/g, '').trim().substring(0, 100);

        console.log(`Análisis Heridas para ${cleanName} - Detectado: ${cleanWound}`);
...
Herida dominante detectada: ${cleanWound}
...
`;
        ...
        if (cleanEmail && cleanEmail.includes('@')) {
            try {
                console.log(`[Heridas Test] Sending email to ${cleanEmail}...`);
                ...
                const pdfBuffer = await generateGenericTestPDF(cleanName, 'Heridas de la Infancia', parsedResult.pdf_content);
                ...
                console.log(`[Heridas Test] Email sent successfully`);
            } catch (mailError) {
                console.error('[Heridas Test Mail Error]:', mailError);
            }
        }
