...
export async function POST(req: NextRequest) {
    try {
        ...
        if (userEmail && userEmail.includes('@')) {
            try {
                console.log(`[Standalone Test Debug] Attempting email to: ${userEmail}, name: ${userName}, test: ${testId}`);
                ...
                const pdfBuffer = await generateGenericTestPDF(userName, `Test: ${testId}`, pdfContent);
                ...
                console.log(`[Standalone Test Debug] Mail Result:`, JSON.stringify(mailResult));
            } catch (mailError) {
                console.error('[Standalone Test Mail Error]:', mailError);
            }
        } else {
            console.warn(`[Standalone Test Debug] Skipping email: invalid or missing email address: "${userEmail}"`);
        }
